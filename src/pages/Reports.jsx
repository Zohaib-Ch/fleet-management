...
const ReportsPage = () => {
  ...
  const [mobileView, setMobileView] = useState('catalog') // 'catalog' or 'dashboard'
  ...
  const renderFleetOverview = () => (
    <div className="space-y-6 lg:space-y-8 pb-10">
      ...
      <div className="bg-white rounded-[1.5rem] lg:rounded-[2.5rem] p-5 lg:p-8 border border-slate-100 shadow-sm">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-8">
           <div className="flex flex-col lg:flex-row lg:items-center gap-4">
              <h3 className="text-lg lg:text-xl font-black text-slate-800 tracking-tight">Hourly Charts</h3>
              <div className="bg-slate-50 border border-slate-100 px-3 py-1.5 rounded-lg flex items-center gap-2 w-fit">
                 <Calendar className="w-3.5 h-3.5 text-blue-500" />
                 <span className="text-[10px] lg:text-[11px] font-bold text-slate-600">28.05.2026</span>
              </div>
           </div>
           <p className="text-[9px] lg:text-[10px] font-black text-slate-400 uppercase tracking-widest">Last updated at <span className="text-slate-800">11:00 AM</span></p>
        </div>

        <div className="grid grid-cols-12 gap-6 lg:gap-8">
          {/* Hourly Fleet Status Bar Chart */}
          <div className="col-span-12 lg:col-span-6 bg-slate-50/50 rounded-3xl p-5 lg:p-6 border border-slate-50">
             <SectionLabel>Hourly Fleet Status</SectionLabel>
             <div className="h-[240px] lg:h-[280px] mt-4">
                <ResponsiveContainer width="100%" height="100%">
                   <BarChart data={mockChartData.hourlyFleetStatus}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#E2E8F0" />
                      <XAxis dataKey="h" axisLine={false} tickLine={false} tick={{fontSize: 9, fill: '#94A3B8'}} />
                ...
                {[
                  ...
                  { l: 'Service', c: 'bg-[#8B5CF6]' },
                   ...
                   <div key={item.l} className="flex items-center gap-2">
                      <div className={`w-2.5 h-2.5 lg:w-3 lg:h-3 rounded-full ${item.c}`} />
                      <span className="text-[9px] lg:text-[10px] font-bold text-slate-400 uppercase tracking-tight">{item.l}</span>
                   </div>
                ))}
             </div>
          </div>
          ...
          <div className="col-span-12 lg:col-span-3 bg-slate-50/50 rounded-3xl p-5 lg:p-6 border border-slate-50 flex flex-col">
      ...
      <div className="bg-white rounded-[1.5rem] lg:rounded-[2.5rem] p-5 lg:p-8 border border-slate-100 shadow-sm">
        <div className="flex flex-col lg:flex-row lg:items-center gap-4 lg:gap-6 mb-8">
           <h3 className="text-lg lg:text-xl font-black text-slate-800 tracking-tight">Charts for period</h3>
           <div className="bg-slate-50 border border-slate-100 px-3 py-1.5 rounded-lg flex items-center gap-2 w-fit">
              <Calendar className="w-3.5 h-3.5 text-blue-500" />
              <span className="text-[10px] lg:text-[11px] font-bold text-slate-600">28.04.2026 - 28.05.2026</span>
           </div>
        </div>

        <div className="grid grid-cols-12 gap-6 lg:gap-8">
           {/* Daily Fleet Status composition */}
           <div className="col-span-12 lg:col-span-9 bg-slate-50/50 rounded-3xl p-5 lg:p-6 border border-slate-50">
              <SectionLabel>Daily Fleet Status composition</SectionLabel>
              <div className="h-[280px] lg:h-[320px] mt-4">
                 <ResponsiveContainer width="100%" height="100%">
              ...
              <div className="flex flex-wrap justify-center gap-4 lg:gap-8 mt-4">
                 {[
                   ...
                   <div key={item.l} className="flex items-center gap-2">
                      <div className={`w-2.5 h-2.5 lg:w-3 lg:h-3 rounded-full ${item.c}`} />
                      <span className="text-[9px] lg:text-[10px] font-bold text-slate-400 uppercase tracking-widest">{item.l}</span>
                   </div>
                 ))}
              </div>
           </div>
  ...
  const renderUserIntelligenceReport = () => {
    ...
    return (
      <div className="space-y-6 pb-20">
        ...
        <div className="bg-white rounded-[2rem] lg:rounded-[3rem] p-6 lg:p-10 border border-slate-100 shadow-sm relative overflow-hidden mb-6">
           ...
           <div className="flex flex-col lg:flex-row lg:items-start gap-6 lg:gap-8">
              <div className="w-24 h-24 lg:w-32 lg:h-32 rounded-[2rem] lg:rounded-[2.5rem] overflow-hidden border-4 border-white shadow-xl relative shrink-0">
                 <img src={user.photo} alt="" className="w-full h-full object-cover" />
                 <div className="absolute bottom-0 right-0 w-7 h-7 lg:w-8 lg:h-8 bg-tech-blue border-4 border-white rounded-full flex items-center justify-center">
                    <Star className="w-3.5 h-3.5 text-white fill-white" />
                 </div>
              </div>
              <div className="flex-1">
                 <div className="flex flex-wrap items-center gap-3 mb-2">
                    <h2 className="text-2xl lg:text-4xl font-black text-slate-800 tracking-tight">{user.name}</h2>
                    <span className="px-2 py-0.5 lg:px-3 lg:py-1 bg-emerald-50 text-emerald-600 rounded-lg text-[9px] lg:text-[10px] font-black uppercase tracking-widest">Master Driver</span>
                 </div>
                 <p className="text-slate-400 text-xs lg:text-sm font-medium max-w-xl leading-relaxed mb-6">Intelligence synthesis for {user.role.name}. Correlation of telemetry, safety compliance, and efficiency metrics.</p>
                 
                 <div className="grid grid-cols-2 lg:grid-cols-4 gap-y-6 gap-x-4">
                    <div>
                       <p className="text-[9px] lg:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Safety Score</p>
                       <p className="text-xl lg:text-2xl font-black text-slate-800">{user.performance}%</p>
                    </div>
                    <div className="lg:border-l lg:border-slate-100 lg:pl-6">
                       <p className="text-[9px] lg:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">On-Time Rate</p>
                       <p className="text-xl lg:text-2xl font-black text-slate-800">98.4%</p>
                    </div>
                    <div className="border-l border-slate-100 pl-4 lg:pl-6">
                       <p className="text-[9px] lg:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Fuel Economy</p>
                       <p className="text-xl lg:text-2xl font-black text-blue-600">32.8L</p>
                    </div>
                    <div className="border-l border-slate-100 pl-4 lg:pl-6">
                       <p className="text-[9px] lg:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Assets Managed</p>
                       <p className="text-xl lg:text-2xl font-black text-slate-800">12</p>
                    </div>
                 </div>
              </div>
           </div>
        ...
        <div className="grid grid-cols-12 gap-6">
           ...
           <div className="col-span-12 lg:col-span-8 space-y-6">
              <motion.div {...fadeUp(0.1)} className="bg-white rounded-[2rem] lg:rounded-[2.5rem] p-6 lg:p-8 border border-slate-100 shadow-sm">
                 <SectionLabel>Weekly Performance Synthesis</SectionLabel>
                 <div className="h-56 lg:h-64 mt-4">
                    <ResponsiveContainer width="100%" height="100%">
                       <AreaChart data={mockChartData.fuelWeekly}>
                          ...
                          <XAxis dataKey="h" axisLine={false} tickLine={false} tick={{fontSize: 9, fill: '#94A3B8'}} />
                          <Tooltip contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 20px 50px rgba(0,0,0,0.1)' }} />
                          <Area type="monotone" dataKey="v" stroke="#3B82F6" strokeWidth={3} fill="url(#userGrad)" />
                       </AreaChart>
                    </ResponsiveContainer>
                 </div>
              </motion.div>

              <motion.div {...fadeUp(0.2)} className="bg-white rounded-[2rem] lg:rounded-[2.5rem] p-6 lg:p-8 border border-slate-100 shadow-sm">
                 <SectionLabel>Recent Activity Log</SectionLabel>
                 <div className="space-y-3 lg:space-y-4">
                    {[
                       { title: 'Geofence Entry: Zone A', date: 'May 5, 2026', type: 'System', val: 'Verified' },
                       { title: 'Route Optimization Applied', date: 'May 5, 2026', type: 'AI', val: '-12%' },
                       { title: 'Pre-Trip Inspection Complete', date: 'May 4, 2026', type: 'Safety', val: 'Passed' },
                    ].map((log, i) => (
                       <div key={i} className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-50 group hover:bg-white transition-all">
                          <div className="flex items-center gap-3 lg:gap-4 min-w-0">
                             <div className="w-9 h-9 lg:w-10 lg:h-10 rounded-xl bg-white flex items-center justify-center text-blue-500 shadow-sm shrink-0">
                                <Activity className="w-4 h-4 lg:w-5 lg:h-5" />
                             </div>
                             <div className="min-w-0">
                                <p className="text-sm font-black text-slate-800 truncate">{log.title}</p>
                                <p className="text-[9px] lg:text-[10px] text-slate-400 font-bold uppercase tracking-tight truncate">{log.date} · {log.type}</p>
                             </div>
                          </div>
                          <span className="text-[9px] font-black px-2 py-1 bg-white border border-slate-100 rounded-lg text-slate-600 uppercase tracking-widest shrink-0 ml-2">{log.val}</span>
                       </div>
                    ))}
                 </div>
              </motion.div>
           ...
           <div className="col-span-12 lg:col-span-4 space-y-6">
              <motion.div {...fadeUp(0.3)} className="bg-white rounded-[2rem] lg:rounded-[2.5rem] p-6 lg:p-8 border border-slate-100 shadow-sm">
              ...
              <motion.div {...fadeUp(0.4)} className="bg-slate-900 rounded-[2rem] lg:rounded-[2.5rem] p-6 lg:p-8 shadow-2xl relative overflow-hidden group">
                 ...
                 <p className="text-[9px] lg:text-[10px] font-black text-blue-400 uppercase tracking-[0.2em] mb-6">Achievements</p>
                 <div className="grid grid-cols-2 gap-3 lg:gap-4 relative z-10">
                    {[
                       ...
                       <div key={i} className="flex flex-col items-center p-3 lg:p-4 rounded-2xl lg:rounded-3xl bg-white/5 border border-white/10 text-center group hover:bg-white/10 transition-all">
                          <div className={`w-10 h-10 lg:w-12 lg:h-12 rounded-xl lg:rounded-2xl ${ach.bg} flex items-center justify-center ${ach.color} mb-3 group-hover:scale-110 transition-transform`}>
                             <ach.icon className="w-5 h-5 lg:w-6 lg:h-6" />
                          </div>
                          <p className="text-[9px] lg:text-[10px] font-black text-white uppercase tracking-tighter">{ach.label}</p>
                       </div>
                    ))}
                 </div>
              </motion.div>
  ...
  const SectionLabel = ({ children }) => <p className="text-[10px] lg:text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] mb-6">{children}</p>
    ...
    <div className="flex flex-col h-screen w-screen bg-[#F8FAFC] overflow-hidden">
      ...
      <main className="flex-1 flex flex-col overflow-hidden min-w-0 p-3 pt-24 gap-3">
        ...
        <div className="flex-1 flex gap-3 overflow-hidden min-h-0">
          ...
          <div className={`
            ${mobileView === 'catalog' ? 'flex' : 'hidden'} 
            lg:flex w-full lg:w-[300px] shrink-0 flex-col bg-white rounded-[2rem] lg:rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden h-full
          `}>
             <div className="p-5 lg:p-6 border-b border-slate-50">
                ...
                <p className="text-[9px] lg:text-[10px] text-slate-400 font-bold uppercase tracking-wider leading-relaxed">Intelligence catalog — select a report to analyze fleet data.</p>
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar p-3">
                 <div className="flex gap-2 mb-4 px-2 overflow-x-auto no-scrollbar">
                    {['All 40', 'Favorites', 'Recent'].map(tab => (
                       <button key={tab} className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider whitespace-nowrap ${tab === 'All 40' ? 'bg-slate-800 text-white' : 'bg-slate-50 text-slate-400 hover:text-slate-600'}`}>{tab}</button>
                    ))}
                 </div>
              ...
              <div className="p-4 border-t border-slate-50 bg-slate-50/30">
                 <button className="w-full py-3 bg-white border border-slate-100 rounded-xl text-[9px] lg:text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-blue-600 transition-all flex items-center justify-center gap-2">
           ...
           <div className={`
             ...
             lg:flex flex-1 flex-col gap-3 overflow-hidden
           `}>
              
              {/* Report Toolbar */}
              <div className="bg-white rounded-xl lg:rounded-2xl px-4 lg:px-6 py-3 border border-slate-100 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-4 shrink-0 overflow-x-auto no-scrollbar">
                 <div className="flex items-center gap-3 lg:gap-4">
                    <div className="flex items-center gap-2 text-slate-400 shrink-0">
                       <Clock className="w-3.5 h-3.5" />
                       <span className="text-[9px] lg:text-[10px] font-bold uppercase tracking-wider">Just Now</span>
                    </div>
                    <div className="h-4 w-px bg-slate-100" />
                    <div className="flex items-center gap-1 bg-slate-50 px-2.5 py-1.5 rounded-lg border border-slate-100 cursor-pointer hover:bg-slate-100 transition-all shrink-0">
                       <Calendar className="w-3.5 h-3.5 text-blue-600" />
                       <span className="text-[10px] lg:text-[11px] font-black text-slate-700">Last 14 days</span>
                 ...
                 <div className="flex items-center justify-between lg:justify-end gap-2">
                    <div className="flex items-center">
                       ...
                       <button onClick={() => window.print()} className="p-2 text-slate-300 hover:text-slate-600 transition-all hidden sm:block"><Printer className="w-4 h-4" /></button>
                    ...
                    <button onClick={handleExportWord} className="px-4 lg:px-5 py-2 bg-tech-blue text-white rounded-lg lg:rounded-xl text-[9px] lg:text-[10px] font-black uppercase tracking-[0.1em] shadow-lg shadow-blue-100 hover:bg-blue-700 transition-all flex items-center gap-2 shrink-0">
              ...
              {/* Report Content */}
              <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 lg:pr-2 min-h-0" ref={reportRef}>
                 <AnimatePresence mode="wait">
                    ...
                    >
                       <div className="mb-6 lg:mb-8 flex flex-col lg:flex-row lg:items-end justify-between gap-4">
                         <div>
                           <p className="text-[9px] lg:text-[10px] font-black text-blue-600 uppercase tracking-[0.25em] mb-1">
                           ...
                           <p className="text-xs lg:text-sm text-slate-400 font-medium mt-1">
                             {userId 
                               ...
                               : `Analysis of ${activeReport.split('-')[0]} metrics, alerts, and historical performance trends.`
                             }
                           </p>
                         </div>
                         <div className="flex gap-2">
                           <button 
                             ...
                             className="lg:hidden px-4 py-2 bg-slate-800 text-white rounded-xl text-[9px] font-black uppercase tracking-widest transition-all flex items-center gap-2"
                           >
                             <List className="w-3.5 h-3.5" /> Catalog
                           </button>
                           {userId && (
                             <button 
                               onClick={() => navigate('/reports')}
                               className="px-4 py-2 bg-slate-50 hover:bg-slate-100 border border-slate-100 rounded-xl text-[9px] lg:text-[10px] font-black uppercase tracking-widest text-slate-500 transition-all flex items-center gap-2"
                             >
                       ...
                       {!userId && activeReport !== 'fleet-overview' && activeReport !== 'status-summary' && activeReport !== 'submission-list' && (
                         <div className="p-12 lg:p-20 text-center bg-white rounded-[2rem] lg:rounded-[3rem] border border-dashed border-slate-200">
                            <BarChart3 className="w-12 h-12 lg:w-16 lg:h-16 text-slate-200 mx-auto mb-4" />
                            <h3 className="text-lg lg:text-xl font-black text-slate-400">Advanced Analytics Hub</h3>
                            <p className="text-xs lg:text-sm text-slate-300 max-w-sm mx-auto mt-2 italic font-medium">Detailed data modeling for "{activeReport}" is being calculated. Check back in a moment for full visualization.</p>
                         </div>
                       )}
                    </motion.div>
                 </AnimatePresence>
