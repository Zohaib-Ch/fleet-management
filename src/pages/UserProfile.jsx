...
const UserProfile = () => {
  ...
  const [selectedUser, setSelectedUser] = useState(() => mockUsers.find(u => u.id === id) || mockUsers[0])

  // Sync with URL changes if needed
  useEffect(() => {
  ...
  const handleUserSelect = (user) => {
    ...
    // Update URL without full reload to maintain the selection in history
    ...
    <div className="flex h-screen w-screen bg-[#F0F4F8] overflow-hidden">
      ...
      <main className="flex-1 flex flex-col overflow-hidden min-w-0 p-3 gap-2">
        ...
        <div className="flex gap-3 mb-1">
          <div className="bg-white rounded-2xl p-4 flex-1 border border-slate-100 flex items-center gap-4 shadow-sm">
            ...
            <div>
              ...
              <p className="text-xl font-black text-slate-800 leading-none">{mockUsers.length}</p>
            </div>
            <div className="ml-auto flex gap-4 pr-4">
              <div className="text-right">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">Avg Performance</p>
                <p className="text-sm font-black text-emerald-600 leading-none">92.4%</p>
              </div>
              <div className="text-right border-l pl-4 border-slate-100">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">On Duty</p>
                <p className="text-sm font-black text-blue-600 leading-none">42 Members</p>
              </div>
            </div>
            <button className="h-10 px-4 bg-tech-blue text-white rounded-xl text-xs font-bold flex items-center gap-2 shadow-lg shadow-blue-100">
        ...
        <div className="flex-1 flex gap-3 overflow-hidden min-h-0">
          ...
          {/* Right Panel: Deep-Dive User Information */}
                  ...
                  <div className="col-span-4 space-y-5">
                    <motion.div {...fadeUp(0.12)} className="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm flex flex-col items-center">
                      ...
                      <div className="flex gap-8 mb-6 mt-2">
                        <Ring value={u.performance} max={100} color={perfColor} label="Index Score" />
                    ...
                    <motion.div {...fadeUp(0.16)} className="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm">
                      <SectionLabel>Management Actions</SectionLabel>
                      <div className="grid grid-cols-2 gap-2">
                        <button onClick={() => toast.success('Training assigned')} className="flex flex-col items-center gap-1.5 p-3 rounded-2xl bg-blue-50 hover:bg-blue-100 transition-all border border-blue-100 text-blue-700 group">
                          ...
                          <span className="text-[10px] font-bold">Assign Training</span>
                        </button>
                        <button onClick={() => toast.success('Audit triggered')} className="flex flex-col items-center gap-1.5 p-3 rounded-2xl bg-purple-50 hover:bg-purple-100 transition-all border border-purple-100 text-purple-700">
                          ...
                          <span className="text-[10px] font-bold">Full Audit</span>
                        </button>
                      </div>
                    </motion.div>
                  </div>
                  ...
                  <div className="col-span-8 space-y-5">
                    <div className="grid grid-cols-3 gap-4">
                      {[
                        ...
                        { label: 'Rating', val: '4.9 / 5.0', icon: Star, color: 'text-emerald-500', bg: 'bg-emerald-50' }
                      ].map((item, i) => (
                    ...
                    <motion.div {...fadeUp(0.22)} className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100 flex-1">
                      <h3 className="text-lg font-black text-slate-800 mb-8">Professional Activity Timeline</h3>
                      <div className="space-y-8 relative before:absolute before:left-[19px] before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-50">
                        {[
                          { title: 'Shift Started: Logistic Operations', desc: 'System login authenticated from Terminal 4-A', time: 'Today • 08:30 AM', icon: Zap, color: 'text-blue-500', bg: 'bg-blue-100' },
                          { title: 'Performance Review Completed', desc: 'Quarterly review processed. Grade: Exceptional.', time: 'Yesterday • 04:15 PM', icon: Star, color: 'text-amber-500', bg: 'bg-amber-100' },
                          { title: 'Asset Re-assignment', desc: 'Primary asset updated to VOLVO-FH16', time: '2 days ago • 10:20 AM', icon: Truck, color: 'text-purple-500', bg: 'bg-purple-100' },
                        ].map((ev, i) => (
