...
const VehicleRow = React.memo(({ vehicle, isFocused, isSelected, onSingleClick, onDoubleClick }) => {
  ...
  return (
    <motion.div
      ...
      className={`group relative flex flex-col gap-2 p-3 rounded-2xl cursor-pointer transition-all duration-300 mb-2 border ${isSelected ? 'bg-blue-600 border-blue-500 shadow-premium text-white' :
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="relative shrink-0">
          <div className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all ${isSelected ? 'bg-white/20' : 'bg-slate-50 group-hover:bg-blue-50'
            }`}>
            <VehicleIcon className={`w-4 h-4 ${isSelected ? 'text-white' : 'text-slate-500 group-hover:text-blue-600'}`} />
          </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1">
              <span className={`text-[7px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded-md ${isSelected ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-500'
                }`}>
                {vehicle.type}
              </span>
              <span className={`text-[8px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded-md border ${isSelected ? 'bg-white/10 border-white/20 text-white' : 'bg-slate-50 border-slate-100 text-slate-400'
                }`}>
                {vehicle.id}
              </span>
            </div>
      ...
      <div className="flex items-center justify-between pt-2 border-t border-black/5 mt-1">
        ...
        <div className={`px-1.5 py-0.5 rounded-lg text-[8px] font-black uppercase tracking-tighter ${isSelected ? 'bg-white/20 text-white' : 'bg-slate-50 text-slate-400 border border-slate-100'
          }`}>
          {status.label}
        </div>
      </div>
  ...
  return (
    <div className="mb-2">
      <motion.div
        ...
        className={`flex items-center gap-3 px-3 py-2.5 rounded-2xl cursor-pointer transition-all border ${isActive ? 'bg-white border-blue-200 shadow-premium' : 'bg-white/20 border-transparent hover:bg-white/40'
          }`}
      >
