...
const PageCustomizer = ({ page }) => {
  const { settings, toggleSetting, updateSetting } = useSettings()
  const [isExpanded, setIsExpanded] = useState(false)

  const config = {
    dashboard: [
      { key: 'showKPICards', label: 'KPIs' },
      { key: 'showMap', label: 'Map' },
      { key: 'showHealthCircle', label: 'Health' },
      { key: 'showActionRequired', label: 'Actions' },
      { key: 'showIndustryWidgets', label: 'Specialized' },
      { key: 'showVehicleList', label: 'Fleet List' },
    ],
    vehicles: [
      { key: 'showVehicleKPIs', label: 'KPIs' },
      { key: 'showVehicleGrid', label: 'Management Grid' },
      { key: 'showMaintenanceLog', label: 'Maintenance Log' },
    ],
    users: [
      { key: 'showUserKPIs', label: 'KPIs' },
      { key: 'showUserGrid', label: 'Personnel Grid' },
      { key: 'showAuditLog', label: 'Audit Log' },
    ],
    reports: [
      { key: 'showReportKPIs', label: 'Intelligence KPIs' },
      { key: 'showIndustryWidgets', label: 'Specialized Widgets' },
      { key: 'showConsumptionAnalysis', label: 'Consumption Audit' },
    ]
  }

  const widgets = config[page] || []
    ...
    <div className="flex flex-col gap-2 mb-2">
      ...
      >
        <div className="flex items-center gap-2 px-4 border-r border-slate-200/50 mr-2">
           ...
           <div>
              <p className="text-[9px] font-black text-tech-slate uppercase tracking-tighter leading-none">{page}</p>
              <p className="text-[8px] text-slate-400 font-bold uppercase tracking-widest leading-none">Layout</p>
           </div>
        </div>
