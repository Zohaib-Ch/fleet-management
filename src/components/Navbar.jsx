...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-[5000] transition-all duration-500 px-6 py-3 ${isScrolled ? 'top-2' : 'top-0'}`}>
      <div className={`max-w-[1600px] mx-auto transition-all duration-500 ${isScrolled ? 'rounded-[2.5rem] bg-white/80 backdrop-blur-2xl shadow-[0_20px_50px_rgba(0,0,0,0.08)] border border-white/50 px-8 py-3' : 'bg-transparent px-2 py-4'}`}>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              <button
                ...
                className={`w-11 h-11 rounded-2xl flex items-center justify-center transition-all ${showNotifications ? 'bg-blue-600 text-white shadow-lg' : 'bg-white border border-slate-100 text-slate-400 hover:border-blue-200 hover:text-blue-600 shadow-sm'}`}
              >
                ...
                {showNotifications && (
                  <>
                    ...
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0, scale: 1 }}
                      exit={{ opacity: 0, y: 15, scale: 0.95 }}
                      className="absolute right-0 mt-3 w-80 bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden z-20"
                    >
                      ...
                      <div className="p-3">
                        <button
                          ...
                          className="w-full py-3 bg-slate-50 hover:bg-blue-50 text-blue-600 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all"
                        >
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-3 pl-2 pr-4 py-1.5 bg-white border border-slate-100 rounded-2xl hover:border-blue-200 transition-all shadow-sm group"
              >
                ...
                <ChevronDown className={`w-3.5 h-3.5 text-slate-300 transition-transform duration-300 ${showProfile ? 'rotate-180' : ''}`} />
              </button>
                ...
                {showProfile && (
                  <>
                    ...
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0, scale: 1 }}
                      exit={{ opacity: 0, y: 15, scale: 0.95 }}
                      className="absolute right-0 mt-3 w-64 bg-white rounded-[2rem] shadow-2xl border border-slate-100 p-3 z-20 overflow-hidden"
                    >
            ...
            <button
              ...
              className="xl:hidden w-11 h-11 rounded-2xl bg-white border border-slate-100 flex items-center justify-center text-slate-600"
            >
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            ...
            className="xl:hidden mt-2 bg-white rounded-[2rem] shadow-2xl border border-slate-100 overflow-hidden"
          >
            <div className="p-6 grid grid-cols-2 gap-3">
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`flex flex-col items-center gap-2 p-4 rounded-2xl border transition-all ${isActive(link.path) ? 'bg-blue-600 border-blue-600 text-white' : 'bg-slate-50 border-slate-100 text-slate-500'}`}
                >
